...
def abrir_ventana_principal():
    ...
    frame1 = tk.Frame(ventanaPrincipal)
    frame1.grid(row=0, column=0)
    etiqueta1 = tk.Label(frame1, text="Calculadora de emisiones:")
    etiqueta1.grid(row=0, column=0)

    boton1 = tk.Button(frame1, text="CALCULAR", command=lambda: obtener_datos_usuario(obtener_nombre_por_voz()))
    boton1.grid(row=1, column=0)

    frame2 = tk.Frame(ventanaPrincipal)
    frame2.grid(row=0, column=1)
    cajaTexto1 = tk.Text(frame2, height=15, width=80)
